...
func (cfg *Config) NewNode(ctx context.Context, logger *zap.Logger) (host.Host, error) {
	...
	if cfg.EnableAutoRelay {
		...
		if hop {
			...
			_ = relay.NewAutoRelay(swrm.Context(), logger, h, discovery, router, cfg.StaticRelays)
		}
	}
	...
	if router != nil {
		return routed.Wrap(h, router, logger), nil
	}
	return h, nil
}
