...
func TestBadTransportConstructor(t *testing.T) {
	...
	h, err := New(ctx, zaptest.NewLogger(t), Transport(func() {}))
	if err == nil {
		h.Close()
...
func TestNoListenAddrs(t *testing.T) {
	ctx := context.Background()
	h, err := New(ctx, zaptest.NewLogger(t), NoListenAddrs)
	if err != nil {
		t.Fatal(err)
	}
	defer h.Close()
...
func TestNoTransports(t *testing.T) {
	ctx := context.Background()
	a, err := New(ctx, zaptest.NewLogger(t), NoTransports)
	if err != nil {
		t.Fatal(err)
	}
	defer a.Close()

	b, err := New(ctx, zaptest.NewLogger(t), ListenAddrStrings("/ip4/127.0.0.1/tcp/0"))
	if err != nil {
		t.Fatal(err)
	}
	defer b.Close()
...
func TestInsecure(t *testing.T) {
	ctx := context.Background()
	h, err := New(ctx, zaptest.NewLogger(t), NoSecurity)
	if err != nil {
		t.Fatal(err)
	}
	h.Close()
...
func TestDefaultListenAddrs(t *testing.T) {
	...
	h, err := New(ctx, zaptest.NewLogger(t))
	if err != nil {
		t.Fatal(err)
	}
...
func makeRandomHost(t *testing.T, port int) (host.Host, error) {
	...
	return New(ctx, zaptest.NewLogger(t), opts...)
}
