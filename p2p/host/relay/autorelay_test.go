...
import (
	...
	libp2p "github.com/RTradeLtd/libp2px"
	...
	autonatpb "github.com/RTradeLtd/libp2px/pkg/autonat/pb"
...
func makeAutoNATServicePrivate(ctx context.Context, t *testing.T) host.Host {
	h, err := libp2p.New(ctx, zaptest.NewLogger(t))
	if err != nil {
		t.Fatal(err)
	}
...
func TestAutoRelay(t *testing.T) {
	...
	_, err := libp2p.New(ctx, zaptest.NewLogger(t), libp2p.EnableRelay(circuit.OptHop), libp2p.EnableAutoRelay(), libp2p.Routing(makeRouting))
	if err != nil {
		t.Fatal(err)
	}
	h3, err := libp2p.New(ctx, zaptest.NewLogger(t), libp2p.EnableRelay(), libp2p.EnableAutoRelay(), libp2p.Routing(makeRouting))
	if err != nil {
		t.Fatal(err)
	}
	h4, err := libp2p.New(ctx, zaptest.NewLogger(t), libp2p.EnableRelay())
	if err != nil {
		t.Fatal(err)
	}
